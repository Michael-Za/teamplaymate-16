...
const MatchTracker = ({ onClose }: { onClose: () => void }) => {
  ...
  const players = [
    { id: '1', name: 'DÍAZ', number: 1 },
    { id: '2', name: 'LÓPEZ', number: 2 },
    { id: '3', name: 'GARCÍA', number: 3 },
    { id: '4', name: 'MARTÍN', number: 4 },
    { id: '5', name: 'PÉREZ', number: 5 },
    { id: '6', name: 'SÁNCHEZ', number: 6 },
    { id: '7', name: 'RUIZ', number: 7 },
    { id: '8', name: 'JIMÉNEZ', number: 8 },
    { id: '9', name: 'HERNÁNDEZ', number: 9 },
    { id: '10', name: 'MORENO', number: 10 },
    { id: '11', name: 'MUÑOZ', number: 11 },
    { id: '12', name: 'ÁLVAREZ', number: 12 },
  ];
  ...
  const renderFouls = (count: number) => {
    return Array(5).fill(0).map((_, i) => (
      <div 
        key={i} 
        className={`w-3 h-3 rounded-full border ${i < count ? 'bg-yellow-400' : 'bg-gray-200'}`}
      />
    ));
  };

  const logAction = (action: string, player?: string, team?: 'HOME' | 'AWAY') => {
    const time = formatTime(minutes, seconds);
    setActionLogs(prev => [...prev, { time, player, action, team }]);
  };
  ...
  const exportToPDF = () => {
    try {
      ...
      // Title
      doc.setFontSize(18);
      doc.text('MATCH REPORT', 14, 20);
      
      // Match Information
      doc.setFontSize(12);
      doc.text(`Date: ${new Date().toLocaleDateString()}`, 14, 35);
      doc.text(`Time: ${formatTime(minutes, seconds)}`, 14, 45);
      doc.text(`Score: ${homeScore} - ${awayScore}`, 14, 55);
      doc.text(`Fouls: Home ${homeFouls} - Away ${awayFouls}`, 14, 65);
      
      // Players Section
      doc.setFontSize(14);
      doc.text('PLAYERS:', 14, 80);
      doc.setFontSize(10);
      
      let yPos = 90;
      players.forEach((player, index) => {
        const status = index < 6 ? 'STARTER' : 'SUB';
        doc.text(`#${player.number} ${player.name} - ${status}`, 20, yPos);
        yPos += 8;
      });
      
      // Events Section
      doc.setFontSize(14);
      doc.text('MATCH EVENTS:', 14, yPos + 10);
      doc.setFontSize(10);
      ...
      if (actionLogs.length > 0) {
        actionLogs.forEach((log) => {
          ...
          const eventText = `${log.time} - ${log.team || ''} ${log.player || ''} ${log.action}`;
          doc.text(eventText, 20, yPos);
          yPos += 8;
        });
      } else {
        doc.text('No events recorded', 20, yPos);
      }
      
      // Save the PDF
      const filename = `match-report-${new Date().toISOString().slice(0,10)}.pdf`;
      doc.save(filename);
  ...
  const handleFoul = (team: 'HOME' | 'AWAY') => {
    ...
    logAction('FOUL', undefined, team);
  };

  const handlePlayerAction = (playerName: string, action: string) => {
    logAction(action, playerName, 'HOME');
  };
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 p-2 overflow-auto">
      <div className="max-w-6xl mx-auto h-full flex flex-col">
        {/* Header */}
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-xl font-bold text-gray-900">MATCH TRACKER</h2>
          <div className="flex gap-2">
            <Button 
              ...
              variant="outline" 
              className="px-3 py-1 text-xs border-2 border-gray-300 font-semibold text-gray-800 hover:bg-gray-50 flex items-center gap-1"
            >
            ...
            <Button 
              onClick={onClose} 
              variant="outline" 
              className="px-3 py-1 text-xs border-2 border-gray-300 font-semibold text-gray-800 hover:bg-gray-50"
            >
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-3">
          ...
          <div className="space-y-3">
            {/* Main Actions */}
            <div>
              <h3 className="text-sm font-bold text-gray-800 mb-2">MAIN ACTIONS</h3>
              <div className="grid grid-cols-2 gap-2 w-full">
                <Button 
                  className="bg-green-600 hover:bg-green-700 h-12 w-full text-white font-medium text-sm rounded" 
                ...
                <Button 
                  className="bg-blue-600 hover:bg-blue-700 h-12 text-white font-medium text-sm rounded" 
                  onClick={() => handleFoul('HOME')}
                >
                  <AlertCircle className="mr-1 h-4 w-4" />
                  FOUL FOR
                </Button>
                <Button className="bg-blue-600 hover:bg-blue-700 h-12 w-full text-white font-medium text-sm rounded">
                  <CornerDownRight className="mr-1 h-4 w-4" />
                  CORNER FOR
                </Button>
                <Button 
                  className="bg-red-600 hover:bg-red-700 h-12 text-white font-medium text-sm rounded" 
                ...
                <Button 
                  className="bg-orange-500 hover:bg-orange-600 h-12 w-full text-white font-medium text-sm rounded" 
                  onClick={() => handleFoul('AWAY')}
                >
                  <AlertCircle className="mr-1 h-4 w-4" />
                  FOUL AGAINST
                </Button>
                <Button className="bg-blue-600 hover:bg-blue-700 h-12 text-white font-medium text-sm rounded">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 w-full">
                <Button variant="outline" className="h-10 w-full text-xs font-medium rounded border border-gray-300 hover:bg-gray-50" onClick={() => logAction('DUEL WON')}>
                  <Shield className="mr-1 h-3 w-3" />
                  DUEL WON
                </Button>
                <Button variant="outline" className="h-10 text-xs font-medium rounded border border-gray-300 hover:bg-gray-50" onClick={() => logAction('BALL LOST')}>
                  <Zap className="mr-1 h-3 w-3" />
                  BALL LOST
                </Button>
                <Button variant="outline" className="h-10 text-xs font-medium rounded border border-gray-300 hover:bg-gray-50" onClick={() => logAction('SHOT ON TARGET')}>
                  <Target className="mr-1 h-3 w-3" />
                  SHOT ON TGT
                </Button>
                <Button variant="outline" className="h-10 text-xs font-medium rounded border border-gray-300 hover:bg-gray-50" onClick={() => logAction('DUEL LOST')}>
                  <Shield className="mr-1 h-3 w-3" />
                  DUEL LOST
                </Button>
                <Button variant="outline" className="h-10 text-xs font-medium rounded border border-gray-300 hover:bg-gray-50" onClick={() => logAction('BALL RECOVERED')}>
                  <Zap className="mr-1 h-3 w-3" />
                  BALL RECOVERED
                </Button>
                <Button variant="outline" className="h-10 text-xs font-medium rounded border border-gray-300 hover:bg-gray-50" onClick={() => logAction('SHOT OFF TARGET')}>
                  <Target className="mr-1 h-3 w-3" />
                  SHOT OFF TGT
                </Button>
                <div className="col-span-3 h-0"></div>
              </div>
            </div>

            {/* Extra Actions */}
            <div>
              <h3 className="text-sm font-bold text-gray-800 mb-2">EXTRA ACTIONS</h3>
              <div className="grid grid-cols-3 gap-2 w-full">
                <Button className="bg-blue-600 hover:bg-blue-700 h-10 w-full text-white text-xs font-medium rounded" onClick={() => logAction('PENALTY FOR')}>
                  <PenLine className="mr-1 h-3 w-3" />
                  PENALTY FOR
                </Button>
                <Button className="bg-orange-500 hover:bg-orange-600 h-10 text-white text-xs font-medium rounded" onClick={() => logAction('PENALTY AGAINST')}>
                  <PenLine className="mr-1 h-3 w-3" />
                  PENALTY AGAINST
                </Button>
                <Dialog open={notesDialogOpen} onOpenChange={setNotesDialogOpen}>
                <DialogTrigger asChild>
                  <Button className="bg-yellow-400 hover:bg-yellow-500 h-10 text-gray-900 text-xs font-medium rounded">
                    <PenLine className="mr-1 h-3 w-3" />
                    NOTES
                  </Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle className="text-lg font-bold">Add Notes</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-4">
                    <textarea
                      ...
                      className="w-full min-h-[100px] p-3 border border-gray-300 rounded-md resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                      autoFocus
                    />
                      ...
                      actionLogs.map((log, index) => (
                        <div key={index} className="flex items-center justify-between py-1 px-2 bg-gray-50 rounded text-xs">
                          ...
                          <div className="flex-1 ml-2">
                            {log.player && <span className="font-semibold text-gray-800">{log.player}: </span>}
                            <span className="text-gray-700">{log.action}</span>
                            {log.team && (
                              <Badge variant={log.team === 'HOME' ? 'default' : 'destructive'} className="ml-1 text-xs px-1 py-0">
                                {log.team}
                              </Badge>
                            )}
                          </div>
          ...
          <div className="lg:col-span-2 space-y-3">
            {/* Scoreboard */}
            <Card className="shadow-sm">
              <CardHeader className="flex flex-row items-center justify-between py-2 px-3 bg-gray-100 border-b">
                <div className="flex items-center space-x-2">
                  <div className="w-6 h-6 bg-orange-500 rounded-full"></div>
                  <span className="text-sm font-bold">SANT JOAN</span>
                </div>
                <Button 
                  variant="destructive" 
                ...
                <div className="text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex justify-center gap-2">
                      <Button 
                        variant="outline" 
                        className="w-24 h-8 text-xs font-medium border border-gray-300"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center">
                      <div className="text-5xl font-bold text-gray-900">{homeScore}</div>
                      <div className="text-sm text-gray-600 font-medium">HOME</div>
                    </div>
                    <div className="text-center">
                      <div className="text-5xl font-bold text-gray-900">{awayScore}</div>
                      <div className="text-sm text-gray-600 font-medium">AWAY</div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-xs font-semibold mb-1 text-gray-700">FOULS</div>
                      <div className="flex justify-center gap-1">
                        {renderFouls(homeFouls)}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold mb-1 text-gray-700">FOULS</div>
                      <div className="flex justify-center gap-1">
                        {renderFouls(awayFouls)}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {/* Players */}
            <div>
              <h3 className="text-sm font-bold text-gray-800 mb-2">PLAYERS</h3>
              <div className="grid grid-cols-3 gap-1.5">
                {players.map((player) => (
                  <Button
                    key={player.id}
                    variant="outline"
                    className="h-14 flex flex-col justify-center items-center p-1 rounded border border-gray-300 hover:bg-gray-50"
                    onClick={() => handlePlayerAction(player.name, 'PLAYER SELECTED')}
                  >
                    <div className="text-xs text-gray-600 font-medium">{player.number}</div>
                    <div className="text-xs font-semibold text-gray-900 truncate w-full">{player.name}</div>
                  </Button>
                ))}
              </div>
            </div>
...
const CommandTable: React.FC = () => {
  ...
  return (
    <div className="p-4 md:p-6">
      {showMatchTracker && <MatchTracker onClose={() => setShowMatchTracker(false)} />}
      {/* Event Modal */}
      {showEventModal && (
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <Circle className="mr-1 h-3 w-3 md:mr-2 md:h-4 md:w-4" />
            <span className="hidden sm:inline">2nd Mode</span>
          </Button>
              ...
              {filteredPlayers.length === 0 && (
                <div className="text-center py-6">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    No players available in the system.
                  </p>
                </div>
              )}
            </CardContent>
