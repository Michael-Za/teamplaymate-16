...
const CommandTable: React.FC = () => {
  ...
  const { 
    ...
    addPlayerAction: contextAddPlayerAction, 
    deletePlayerAction: contextDeletePlayerAction 
  } = useDataSync(); // Use players, playerActions and attendance from DataSyncContext
  
  // Transform DataSync players to CommandTable format
  const transformedPlayers = dataSyncPlayers.map((player: any) => ({
    id: player.id || '',
    name: player.name,
    number: player.jersey_number || player.number || 0,
    position: player.position,
    role: '', // This would need to be mapped from player data
    notes: player.notes || '',
    rating: 3.0, // This would need to be mapped from player data
    status: 'available' as 'available' | 'injured' | 'suspended',
    goals: player.goals || 0,
    assists: player.assists || 0,
    yellowCards: player.yellowCards || 0,
    redCards: player.redCards || 0,
    matchesPlayed: player.games || 0,
    createdAt: new Date(player.created_at || Date.now())
  }));
  
  const [players, setPlayers] = useState<Player[]>(transformedPlayers);
  ...
  const saveEditingPlayer = async () => {
    ...
    try {
      // Update player through DataSyncContext
      await updatePlayer(editingPlayerId, {
        name: editPlayer.name,
        jersey_number: editPlayer.number,
  ...
  const handleTabChange = (direction: 'left' | 'right') => {
    if (direction === 'left' && activeTab !== 'players') {
      setActiveTab(prevTab => (prevTab === 'formation' ? 'players' : 'formation'));
    } else if (direction === 'right' && activeTab !== 'actions') {
      setActiveTab(prevTab => (prevTab === 'formation' ? 'actions' : 'formation'));
    }
  };

  const handleActionScroll = (direction: 'left' | 'right') => {
    if (direction === 'left' && actionScrollPosition > 0) {
      setActionScrollPosition(actionScrollPosition - 100);
    } else if (direction === 'right' && actionsContainerRef.current) {
      const containerWidth = actionsContainerRef.current.scrollWidth;
      const clientWidth = actionsContainerRef.current.clientWidth;
      if (actionScrollPosition < containerWidth - clientWidth) {
        setActionScrollPosition(actionScrollPosition + 100);
      }
    }
  };
  ...
  const handleDeletePlayerAction = async (actionId: string) => {
    ...
    // For attendance-related actions, delete from attendance records
    if (['present', 'absent', 'late'].includes(action.actionType)) {
      try {
        // In a real implementation, we would need to find the specific attendance record
        // associated with this action and delete it
        console.log('Would delete attendance record for player:', action.playerId, 'status:', action.actionType);
      } catch (error) {
        console.error('Error handling attendance record deletion:', error);
        toast.error('Failed to handle attendance record deletion');
      }
    }
  ...
  const recordEventFromModal = async () => {
    try {
      ...
      // For attendance-related actions, add to attendance records
      if (['present', 'absent', 'late'].includes(eventModalData.actionType)) {
        try {
          if (addAttendanceRecord) {
            await addAttendanceRecord({
              ...
              status: eventModalData.actionType as 'present' | 'absent' | 'late',
              notes: eventModalData.details || ''
            });
          }
        } catch (error) {
