...
const PlayerProfile = () => {
  ...
  const handleSave = async () => {
    if (!editedPlayer || !player || !player.id) return;
    
    try {
      // Transform player data for API submission (only DataPlayer properties)
      const playerData: Partial<DataPlayer> = {
        ...editedPlayer,
        date_of_birth: editedPlayer.birthDate ? editedPlayer.birthDate.toISOString().split('T')[0] : undefined
        // Note: photo, height, and weight are not part of the DataPlayer interface
        // so we can't send them to the API
      };
      ...
      if (updatedPlayer) {
        ...
        const transformedPlayer: ExtendedPlayer = {
          ...
          nickname: updatedPlayer.nickname || editedPlayer.nickname || '',
          dominantFoot: updatedPlayer.dominantFoot || editedPlayer.dominantFoot || 'Right',
          birthDate: updatedPlayer.date_of_birth ? new Date(updatedPlayer.date_of_birth) : editedPlayer.birthDate || new Date('1990-01-01'),
          games: updatedPlayer.games || editedPlayer.games || 0,
          yellowCards: updatedPlayer.yellowCards || editedPlayer.yellowCards || 0,
          redCards: updatedPlayer.redCards || editedPlayer.redCards || 0,
          shots: updatedPlayer.shots || editedPlayer.shots || 0,
          shotsOnTarget: updatedPlayer.shotsOnTarget || editedPlayer.shotsOnTarget || 0,
          passes: updatedPlayer.passes || editedPlayer.passes || 0,
          passAccuracy: updatedPlayer.passAccuracy || editedPlayer.passAccuracy || 0,
          foulsCommitted: updatedPlayer.foulsCommitted || editedPlayer.foulsCommitted || 0,
          foulsReceived: updatedPlayer.foulsReceived || editedPlayer.foulsReceived || 0,
          ballsLost: updatedPlayer.ballsLost || editedPlayer.ballsLost || 0,
          ballsRecovered: updatedPlayer.ballsRecovered || editedPlayer.ballsRecovered || 0,
          duelsWon: updatedPlayer.duelsWon || editedPlayer.duelsWon || 0,
          duelsLost: updatedPlayer.duelsLost || editedPlayer.duelsLost || 0,
          crosses: updatedPlayer.crosses || editedPlayer.crosses || 0,
          saves: updatedPlayer.saves || editedPlayer.saves || 0,
          photo: updatedPlayer.photo || editedPlayer.photo || '/placeholder.svg',
        ...
        // If there's a temp photo, save it locally
        if (tempPhoto) {
