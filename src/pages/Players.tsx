...
const Players = () => {
    ...
    if (dataSyncPlayers && dataSyncPlayers.length > 0) {
      ...
      const transformedPlayers = dataSyncPlayers.map((player: any) => {
        ...
        const transformedPlayer: Player = {
          ...
          jersey_number: player.jersey_number || player.number || 0,
  ...
  const loadPlayers = async () => {
    // With DataSyncContext, we don't need to manually load players
    // The data is automatically synced through the context
  };

  const handleAddPlayer = async (newPlayerData: Partial<Player>) => {
    try {
      if (addPlayerToContext) {
