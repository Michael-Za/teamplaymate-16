...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      if (!googleClientId) {
        const configError = 'Google OAuth is not configured properly. Please follow these steps:\n\n' +
          ...
          'For detailed instructions, check the .env.example file.';
        toast.error('Google OAuth Configuration Missing', { duration: 8000 });
        throw new Error(configError);
      }
      
      if (!backendUrl) {
        const backendError = 'Backend API URL is not configured. Please set VITE_BACKEND_URL in your .env file.\n\n' +
          'Example: VITE_BACKEND_URL=http://localhost:3001';
        toast.error('Backend Configuration Missing', { duration: 5000 });
