...
const mockPlayers: Player[] = [
  ...
  { id: '2', name: 'Silva', position: 'CM', jersey_number: 8, age: 26, nationality: 'Brazil' } as Player,
  { id: '3', name: 'Rodriguez', position: 'CB', jersey_number: 4, age: 28, nationality: 'Argentina' } as Player,
  { id: '4', name: 'Martinez', position: 'GK', jersey_number: 1, age: 29, nationality: 'Uruguay' } as Player,
  { id: '5', name: 'Johnson', position: 'RW', jersey_number: 11, age: 25, nationality: 'England' } as Player,
  { id: '6', name: 'Schmidt', position: 'LB', jersey_number: 3, age: 27, nationality: 'Germany' } as Player,
  { id: '7', name: 'Dubois', position: 'CAM', jersey_number: 10, age: 23, nationality: 'France' } as Player,
  { id: '8', name: 'Kowalski', position: 'LW', jersey_number: 7, age: 26, nationality: 'Poland' } as Player
];
...
export const AIAssistantSection: React.FC = () => {
  ...
  useEffect(() => {
    const initializeServices = async () => {
      ...
      // In demo mode, we use mock data, so no need to initialize services
      if (isDemoMode) {
  ...
  const generateChartData = useCallback((query: string, data: Record<string, unknown>): ChartData | null => {
    ...
    if (lowerQuery.includes('performance') || lowerQuery.includes('stats') || lowerQuery.includes('player')) {
      // Generate more realistic player performance data
      const playerNames = ['Messi', 'Ronaldo', 'Neymar', 'Mbappé', 'De Bruyne', 'van Dijk', 'Courtois', 'Haaland', 'Salah', 'Modrić'];
      ...
      return {
        ...
        data: playerNames.map(name => ({
          name: name,
          goals: Math.floor(Math.random() * 35) + 5,
    ...
    if (lowerQuery.includes('match') || lowerQuery.includes('result')) {
      return {
        type: 'line',
        title: 'Match Results Trend',
        data: [
          { name: 'Aug', wins: 5, draws: 1, losses: 0 },
          { name: 'Sep', wins: 6, draws: 1, losses: 1 },
          { name: 'Oct', wins: 5, draws: 2, losses: 1 },
          { name: 'Nov', wins: 4, draws: 1, losses: 2 },
          { name: 'Dec', wins: 3, draws: 2, losses: 1 },
          { name: 'Jan', wins: 5, draws: 0, losses: 1 }
        ],
        insights: [
          'Overall win rate of 72% this season',
          'Home advantage contributing to better results',
          'Recent dip in performance needs attention',
          'Strong start to the new season'
        ]
      };
    }
    
    if (lowerQuery.includes('fitness') || lowerQuery.includes('condition')) {
      return {
        type: 'bar',
        title: 'Player Fitness Metrics',
        data: [
          { name: 'Messi', fitness: 95, injuryRisk: 15, load: 80 },
          { name: 'Ronaldo', fitness: 92, injuryRisk: 25, load: 85 },
          { name: 'Neymar', fitness: 88, injuryRisk: 40, load: 75 },
          { name: 'Mbappé', fitness: 96, injuryRisk: 10, load: 90 },
          { name: 'De Bruyne', fitness: 90, injuryRisk: 20, load: 82 },
          { name: 'van Dijk', fitness: 87, injuryRisk: 18, load: 78 }
        ],
        insights: [
          'Overall team fitness at 91%',
