...
class OpenAIService {
  constructor() {
    this.openai = new OpenAI({
      apiKey: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,
    });
  }
  ...
  async generateChatResponse(messages, context = null) {
    ...
    // If OpenAI is not enabled, return a mock response
    if (!this.enabled) {
      return {
        content: "AI assistant is not configured. Please set OPENAI_API_KEY in your environment variables to enable AI features.",
        tokens_used: 0,
        model_used: "none"
      };
    }
...
Keep explanations step-by-step and simple.`;
  ...
  async generateTrainingRecommendations(playerId, coachId) {
    ...
    if (!this.enabled) {
      return {
        content: "AI training recommendations are not available. Please configure OpenAI API key to enable this feature.",
        tokens_used: 0,
        model_used: "none"
      };
    }
...
Consider recent performance and suggest specific exercises, drills, and improvements.`,
  ...
  async generateMatchAnalysis(matchId, coachId) {
    ...
    if (!this.enabled) {
      return {
        content: "AI match analysis is not available. Please configure OpenAI API key to enable this feature.",
        tokens_used: 0,
        model_used: "none"
      };
    }
...
Provide tactical analysis, key moments, and recommendations for improvement.`,
  ...
  async generatePlayerAnalysis(playerId, coachId) {
    ...
    if (!this.enabled) {
      return {
        content: "AI player analysis is not available. Please configure OpenAI API key to enable this feature.",
        tokens_used: 0,
        model_used: "none"
      };
    }
...
Provide strengths, weaknesses, and improvement areas.`,
  ...
  async generateTeamAnalysis(teamId, coachId) {
    ...
    if (!this.enabled) {
      return {
        content: "AI team analysis is not available. Please configure OpenAI API key to enable this feature.",
        tokens_used: 0,
        model_used: "none"
      };
    }
